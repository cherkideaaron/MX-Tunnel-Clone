...
public final class C1113Lf {
    ...
    public static synchronized void A06(File file) throws IOException {
        if (file.exists()) {
            if (!file.isDirectory()) {
                throw new IOException(A00(FacebookMediationAdapter.ERROR_CREATE_NATIVE_AD_FROM_BID_PAYLOAD, 5, FacebookMediationAdapter.ERROR_BANNER_SIZE_MISMATCH) + file + A00(0, 18, 15));
            }
        } else if (!file.mkdirs()) {
    ...
    public static List<File> A01(File file) {
        ...
        if (fileArrListFiles != null) {
            ...
            Collections.sort(listAsList, new C1112Le());
            return listAsList;
        }
        return linkedList;
    }

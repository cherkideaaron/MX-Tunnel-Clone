...
public abstract class AbstractC2412pU {
    ...
    public static <E> ArrayList<E> A05(Iterator<? extends E> elements) {
        ...
        AbstractC2391p9.A0A(list, elements);
        return list;
    }

    public static boolean A06(@CheckForNull List<?> thisList, Object other) {
        if (other == AbstractC2131ki.A04(thisList)) {
            return true;
        }
        ...
        if (z && (list instanceof RandomAccess)) {
            for (int i = 0; i < size; i++) {
                if (!CB.A01(thisList.get(i), list.get(i))) {
                    return false;
                }
            }
            return true;
        }
        return AbstractC2391p9.A0F(thisList.iterator(), list.iterator());
    }
}

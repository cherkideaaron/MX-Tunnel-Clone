...
public abstract class AbstractC1428Xt {
    ...
    public static String A03(Context context, Throwable th) {
        int iA0M = C1351Up.A0M(context);
        int maxStacktraceLines = C1351Up.A03(context);
        return A06(th, iA0M, maxStacktraceLines, C1351Up.A1h(context));
    }
    ...
    public static String A06(Throwable th, int i, int i2, boolean z) {
        String strA02 = A02(0, 0, FacebookMediationAdapter.ERROR_REQUIRES_UNIFIED_NATIVE_ADS);
        if (th == null) {
            return strA02;
        }
        try {
            QK qk = new QK();
            InterfaceC1426Xr qj = qk;
            String[] strArr = A01;
            ...
            th.printStackTrace(new PrintWriter(new C1427Xs(ql)));
            ql.flush();
            return qk.toString();
    ...
    public static boolean A08(C1425Xq c1425Xq) {
        String middle = c1425Xq.A02();
        if (middle == null) {
            return false;
        }
        ...
        Iterator<String> it = c1425Xq.A00().iterator();
        while (it.hasNext()) {
            if (A0A(it.next())) {
                return true;
            }

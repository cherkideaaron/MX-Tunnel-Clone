...
public class GoogleApiManager implements Handler.Callback {
    ...
    private final Set zaq = new C0569c4(0);
    private final Set zar = new C0569c4(0);
    ...
    public static Status zaJ(ApiKey apiKey, ConnectionResult connectionResult) {
        ...
        return new Status(connectionResult, AbstractC3264qG.m(new StringBuilder(String.valueOf(strZaa).length() + 63 + strValueOf.length()), "API: ", strZaa, " is not available on this device. Connection failed with: ", strValueOf));
    }
    ...
    public final boolean handleMessage(Message message) {
        TaskCompletionSource taskCompletionSourceZab;
        Boolean boolValueOf;
        int i = message.what;
        zabk zabkVar = null;
        switch (i) {
            ...
            case 5:
                ...
                if (zabkVar == null) {
                    Log.wtf("GoogleApiManager", AbstractC3219pQ.e(new StringBuilder(String.valueOf(i2).length() + 65), "Could not find API instance ", i2, " while trying to fail enqueued calls."), new Exception());
                } else if (connectionResult.getErrorCode() == 13) {
                    String errorString = this.zak.getErrorString(connectionResult.getErrorCode());
                    String errorMessage = connectionResult.getErrorMessage();
                    zabkVar.zav(new Status(17, AbstractC3264qG.m(new StringBuilder(String.valueOf(errorString).length() + 69 + String.valueOf(errorMessage).length()), "Error resolution was canceled by the user, original error message: ", errorString, ": ", errorMessage)));
                } else {
            ...
            case 14:
                ...
                if (map5.containsKey(apiKeyZaa)) {
                    boolean zZaw = ((zabk) map5.get(apiKeyZaa)).zaw(false);
                    taskCompletionSourceZab = zaacVar.zab();
                    boolValueOf = Boolean.valueOf(zZaw);
                } else {
                    taskCompletionSourceZab = zaacVar.zab();
                    boolValueOf = Boolean.FALSE;
                }
                taskCompletionSourceZab.setResult(boolValueOf);
                return true;
            case 15:

...
public final class zzeic {
    ...
    public static int zzc(SQLiteDatabase sQLiteDatabase, int i) {
        ...
        if (cursorZzg.getCount() > 0) {
            ...
            i2 = cursorZzg.getInt(cursorZzg.getColumnIndexOrThrow(AppMeasurementSdk.ConditionalUserProperty.VALUE));
        }
        cursorZzg.close();
        return i2;
    }
    ...
    public static long zzd(SQLiteDatabase sQLiteDatabase, int i) {
        ...
        if (cursorZzg.getCount() > 0) {
            cursorZzg.moveToNext();
            j = cursorZzg.getLong(cursorZzg.getColumnIndexOrThrow(AppMeasurementSdk.ConditionalUserProperty.VALUE));
        } else {
            j = 0;
        }
        cursorZzg.close();
    ...
    public static void zzf(SQLiteDatabase sQLiteDatabase, boolean z, boolean z2) throws SQLException {
        ...
        if (z2) {
            ...
            if (z) {
                ...
                str = "UPDATE offline_signal_statistics SET value = value+1 WHERE statistic_name = 'failed_requests'";
            }
        } else {
            str = "UPDATE offline_signal_statistics SET value = value+1 WHERE statistic_name = 'total_requests'";
        }

        ...
        while (true) {
            ...
            if (i3 >= bArr3.length) {
                ...
                if (!MessageDigest.isEqual(zznm.zzc(jArr24), zznm.zzc(jArr25))) {
                    throw new IllegalStateException(AbstractC3264qG.w("Arithmetic error in curve multiplication with the public key: ", zzzx.zza(bArr2)));
                }
                return;
            }
            if (MessageDigest.isEqual(bArr3[i3], bArrCopyOf)) {
                throw new InvalidKeyException(AbstractC3264qG.w("Banned public key: ", zzzx.zza(bArr3[i3])));
            }
            i3++;
        }
    }

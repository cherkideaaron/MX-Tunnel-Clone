...
final class zax implements zabv {
    ...
    private zax(Context context, zaaz zaazVar, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map map, Map map2, ClientSettings clientSettings, Api.AbstractClientBuilder abstractClientBuilder, Api.Client client, ArrayList arrayList, ArrayList arrayList2, Map map3, Map map4) {
        ...
        while (it.hasNext()) {
            z3.put((Api.AnyClientKey) it.next(), this.zad);
        }
        Iterator it2 = map.keySet().iterator();
        while (it2.hasNext()) {
            z3.put((Api.AnyClientKey) it2.next(), this.zae);
        }
        this.zaf = Collections.unmodifiableMap(z3);
    }
    ...
    public static zax zaa(Context context, zaaz zaazVar, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map map, ClientSettings clientSettings, Map map2, Api.AbstractClientBuilder abstractClientBuilder, ArrayList arrayList) {
        ...
        for (Map.Entry entry : map.entrySet()) {
            ...
            if (zRequiresSignIn) {
                z3.put(anyClientKey, client2);
            } else {
                z32.put(anyClientKey, client2);
            }
        }
        Preconditions.checkState(!z3.isEmpty(), "CompositeGoogleApiClient should not be used without any APIs that require sign-in.");
        Z3 z33 = new Z3();
        Z3 z34 = new Z3();
        for (Api api : map2.keySet()) {
            Api.AnyClientKey anyClientKeyZac = api.zac();
            if (z3.containsKey(anyClientKeyZac)) {
                z33.put(api, (Boolean) map2.get(api));
            } else {
                if (!z32.containsKey(anyClientKeyZac)) {
                    throw new IllegalStateException("Each API in the isOptionalMap must have a corresponding client in the clients map.");
                }
                z34.put(api, (Boolean) map2.get(api));
            }
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            Api api2 = zasVar.zaa;
            if (z33.containsKey(api2)) {
                arrayList2.add(zasVar);
            } else {
                if (!z34.containsKey(api2)) {
                    throw new IllegalStateException("Each ClientCallbacks must have a corresponding API in the isOptionalMap");
                }
                arrayList3.add(zasVar);
            }
        }
        return new zax(context, zaazVar, lock, looper, googleApiAvailabilityLight, z3, z32, clientSettings, abstractClientBuilder, client, arrayList2, arrayList3, z33, z34);
    }

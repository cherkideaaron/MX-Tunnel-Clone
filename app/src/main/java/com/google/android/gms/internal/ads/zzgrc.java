...
public final class zzgrc {
    ...
    public static void zzc(boolean z, String str, char c) {
        if (!z) {
            throw new IllegalArgumentException(zzgrt.zzd(str, Character.valueOf(c)));
        }
    }

    public static void zzd(boolean z, String str, int i) {
        if (!z) {
            throw new IllegalArgumentException(zzgrt.zzd(str, Integer.valueOf(i)));
        }
    }

    public static void zze(boolean z, String str, long j) {
        if (!z) {
            throw new IllegalArgumentException(zzgrt.zzd(str, Long.valueOf(j)));
        }
    }
    ...
    public static void zzg(boolean z, String str, int i, int i2) {
        if (!z) {
            throw new IllegalArgumentException(zzgrt.zzd(str, Integer.valueOf(i), Integer.valueOf(i2)));
        }
    }
    ...
    public static int zzm(int i, int i2, String str) {
        ...
        if (i < 0) {
            strZzd = zzgrt.zzd("%s (%s) must not be negative", "index", Integer.valueOf(i));
        } else {
            if (i2 < 0) {
                throw new IllegalArgumentException(AbstractC3264qG.k(new StringBuilder(String.valueOf(i2).length() + 15), "negative size: ", i2));
            }
            strZzd = zzgrt.zzd("%s (%s) must be less than size (%s)", "index", Integer.valueOf(i), Integer.valueOf(i2));
        }
    ...
    public static void zzo(int i, int i2, int i3) {
        if (i < 0 || i2 < i || i2 > i3) {
            throw new IndexOutOfBoundsException((i < 0 || i > i3) ? zzp(i, i3, "start index") : (i2 < 0 || i2 > i3) ? zzp(i2, i3, "end index") : zzgrt.zzd("end index (%s) must not be less than start index (%s)", Integer.valueOf(i2), Integer.valueOf(i)));
        }
    }

    private static String zzp(int i, int i2, String str) {
        if (i < 0) {
            return zzgrt.zzd("%s (%s) must not be negative", str, Integer.valueOf(i));
        }
        if (i2 >= 0) {
            return zzgrt.zzd("%s (%s) must not be greater than size (%s)", str, Integer.valueOf(i), Integer.valueOf(i2));
        }
        throw new IllegalArgumentException(AbstractC3264qG.k(new StringBuilder(String.valueOf(i2).length() + 15), "negative size: ", i2));
    }
}

...
public final class CollectionUtils {
    ...
    public static <K, V> Map<K, V> mapOfKeyValueArrays(K[] kArr, V[] vArr) {
        ...
        if (length != length2) {
            throw new IllegalArgumentException(Vs.n(new StringBuilder(String.valueOf(length).length() + 44 + String.valueOf(length2).length()), "Key and values array lengths not equal: ", length, " != ", length2));
        }
        if (length == 0) {
    ...
    public static <T> Set<T> mutableSetOfWithSize(int i) {
        return i == 0 ? new C0569c4(0) : zza(i, true);
    }
    ...
    private static Set zza(int i, boolean z) {
        if (i <= (true != z ? UserVerificationMethods.USER_VERIFY_HANDPRINT : UserVerificationMethods.USER_VERIFY_PATTERN)) {
            return new C0569c4(i);
        }
        return new HashSet(i, true != z ? 1.0f : 0.75f);
    }

    private static Map zzb(int i, boolean z) {
        return i <= 256 ? new Z3(i) : new HashMap(i, 1.0f);
    }

...
abstract class zzhhe {
    ...
    public zzhhe(byte[] bArr, int i) throws InvalidKeyException {
        if (bArr.length != 32) {
    ...
    public final byte[] zzc(byte[] bArr, ByteBuffer byteBuffer) throws GeneralSecurityException {
        ...
        if (bArr.length != zzb()) {
            int iZzb = zzb();
            throw new GeneralSecurityException(AbstractC3264qG.k(new StringBuilder(String.valueOf(iZzb).length() + 36), "The nonce length (in bytes) must be ", iZzb));
        }
        ...
        for (int i2 = 0; i2 < i + 1; i2++) {
            ...
            zzhwr.zzc(byteBufferAllocate, byteBuffer, byteBufferZzd, i3);
        }
        return byteBufferAllocate.array();
    }

    public final ByteBuffer zzd(byte[] bArr, int i) {
        int[] iArrZza = zza(zzhhc.zzd(bArr), i);

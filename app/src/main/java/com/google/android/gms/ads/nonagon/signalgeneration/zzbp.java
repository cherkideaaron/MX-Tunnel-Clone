...
final class zzbp extends QueryInfoGenerationCallback {
    ...
    public final void onSuccess(QueryInfo queryInfo) throws JSONException {
        ...
        try {
            ...
            String strConcat = ((Boolean) zzbjj.zzc.zze()).booleanValue() ? ",\"as\":".concat(this.zzb.zze().zzb().toString()) : "";
            str = String.format(Locale.getDefault(), "window.postMessage({\"paw_id\":\"%1$s\",\"signal\":\"%2$s\",\"sdk_ttl_ms\":%3$d%4$s}, '*');", this.zza, queryInfo.getQuery(), Long.valueOf(((Boolean) zzbjj.zze.zze()).booleanValue() ? ((Long) zzbjj.zzh.zze()).longValue() : 0L), strConcat);
        }

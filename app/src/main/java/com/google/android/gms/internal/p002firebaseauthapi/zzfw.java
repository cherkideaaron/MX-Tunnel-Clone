...
public final class zzfw extends zzcw {
    ...
    public static zzfw zza(zzfz zzfzVar, zzaal zzaalVar, Integer num) throws GeneralSecurityException {
        ...
        if (zzaVarZzc != zzaVar && num == null) {
            throw new GeneralSecurityException(AbstractC3264qG.x("For given Variant ", String.valueOf(zzfzVar.zzc()), " the value of idRequirement must be non-null"));
        }
        if (zzfzVar.zzc() == zzaVar && num != null) {
            throw new GeneralSecurityException("For given Variant NO_PREFIX the value of idRequirement must be null");
        }
        if (zzaalVar.zza() != 32) {
            throw new GeneralSecurityException(Vs.k(zzaalVar.zza(), "XAesGcmKey key must be constructed with key of length 32 bytes, not "));
        }
